...
import { Menu, X, Ticket, LayoutDashboard, LogOut, CheckCircle2 } from "lucide-react";
...
export function Navbar() {
  ...
  return (
    <>
      <header className={`fixed left-0 right-0 top-0 z-50 transition-all duration-300 ${scrolled ? "glass-nav shadow-2xl backdrop-blur-md bg-zinc-950/80 border-b border-zinc-800/50" : "bg-transparent"}`}>
        <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8" aria-label="Main navigation">
          ...
          <ul className="hidden items-center gap-1 md:flex" role="list">
            {navLinks.map((link) => (
              <li key={link.label}>
                <a href={link.href} target={link.target} rel={link.target === "_blank" ? "noopener noreferrer" : undefined} className="rounded-md px-3 py-2 text-sm text-muted-foreground transition-colors hover:bg-foreground/5 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400">
                  {link.label}
                </a>
              </li>
            ))}
          ...
          <div className="hidden items-center gap-2 md:flex">
            {!user ? (
              <>
                <Link href="/login"><Button variant="ghost" size="sm" className="cursor-pointer text-muted-foreground hover:text-foreground">Sign in</Button></Link>
                <Button size="sm" onClick={() => window.open("https://discord.com/oauth2/authorize?client_id=1303728329689399297&permissions=8&integration_type=0&scope=bot", "_blank", "noopener,noreferrer")} className="cursor-pointer bg-foreground font-medium text-background hover:bg-foreground/90">Add to Discord</Button>
              </>
            ) : (
              <div className="relative" ref={menuRef}>
                <button type="button" onClick={() => setMenuOpen((value) => !value)} aria-label="Open user menu" aria-expanded={menuOpen} aria-haspopup="menu" className="flex items-center gap-2 rounded-full border border-zinc-800 p-1 transition-all hover:border-zinc-700 hover:bg-zinc-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400">
                  <img src={getAvatarUrl(user)} alt="" className="size-8 rounded-full object-cover" />
                </button>

                <AnimatePresence>
                  {menuOpen && (
                    <motion.div initial={{ opacity: 0, y: 6, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 6, scale: 0.96 }} transition={{ duration: 0.15 }} className="absolute right-0 z-50 mt-2 w-52 overflow-hidden rounded-xl border border-zinc-800/90 bg-zinc-950 py-1 shadow-2xl" role="menu">
                      <div className="border-b border-zinc-800/60 bg-zinc-900/40 px-3.5 py-2.5">
                        <div className="flex items-center gap-1.5 text-[11px] font-medium text-emerald-400"><CheckCircle2 className="size-3.5" aria-hidden="true" /><span>Signed in</span></div>
          ...
          <button type="button" className="rounded-md p-2 text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400 md:hidden" aria-label={mobileOpen ? "Close mobile menu" : "Open mobile menu"} aria-expanded={mobileOpen} aria-controls="mobile-navigation" onClick={() => setMobileOpen((value) => !value)}>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div id="mobile-navigation" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="fixed left-0 right-0 top-16 z-40 border-t border-zinc-800/80 bg-zinc-950 px-4 py-4" aria-label="Mobile navigation">
            <ul className="flex flex-col gap-1" role="list">
              {navLinks.map((link) => (
                <li key={link.label}><a href={link.href} target={link.target} rel={link.target === "_blank" ? "noopener noreferrer" : undefined} className="block rounded-md px-3 py-2 text-sm text-muted-foreground transition-colors hover:bg-foreground/5 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400" onClick={() => setMobileOpen(false)}>{link.label}</a></li>
              ))}
            </ul>
              ...
              {!user ? (
                ...
                <>
                  <div className="flex items-center gap-3 rounded-lg border border-zinc-800 bg-zinc-900/60 px-3 py-2">
                    <img src={getAvatarUrl(user)} alt="" className="size-8 rounded-full object-cover" />
                    <div className="flex min-w-0 flex-col"><span className="flex items-center gap-1 text-xs font-medium text-emerald-400"><CheckCircle2 className="size-3" aria-hidden="true" /> Signed in</span><span className="truncate text-xs font-semibold text-zinc-200">{user.username || user.name || "User"}</span></div>
                  </div>
