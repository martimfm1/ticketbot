...
async function markFailed(eventId: string, error: unknown) {
  await createAdminClient().rpc("fail_stripe_webhook_event", {
    p_event_id: eventId,
...
async function syncCheckout(session: Stripe.Checkout.Session) {
  ...
  if (!guildId || !ownerId) throw new BillingError("Checkout session is missing server billing metadata.", "WEBHOOK_PROCESSING_FAILED");
...
export async function POST(request: Request) {
  ...
  if (!signature || !secret) return NextResponse.json({ error: "Invalid webhook request." }, { status: 400 });
  ...
  const { data: claim, error: claimError } = await database.rpc("claim_stripe_webhook_event", {
    p_event_id: event.id,
    p_event_type: event.type,
    p_lease_seconds: 300,
  });

  if (claimError) return NextResponse.json({ error: "Webhook ledger unavailable." }, { status: 500 });
  ...
  try {
    ...
    if (["customer.subscription.created", "customer.subscription.updated", "customer.subscription.deleted"].includes(event.type)) {
      ...
      if (guildId && ownerId) await TicketStripeService.syncFromStripe(guildId, ownerId, subscription);
    }

    await database.rpc("complete_stripe_webhook_event", { p_event_id: event.id });
