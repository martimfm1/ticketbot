...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(
      { roles: roles.map((role) => ({ id: role.id, name: role.name, color: role.color, position: role.position })) },
    ...
    return NextResponse.json(
      { error: status === 401 ? "Unauthorized" : status === 403 ? "Forbidden" : "Failed to load Discord roles" },
      { status },
    );
  }
}
