...
export async function POST(req: Request) {
  try {
    ...
    if (
      ...
      !isValidDiscordId(String(ticket_category_id))
    ) {
    ...
    if (
      ...
      !isValidDiscordId(String(transcript_channel_id))
    ) {
    ...
    if (security_config !== undefined) {
      const serialized = JSON.stringify(security_config);
    ...
    if (ticket_category_id !== undefined) {
      serverPayload.ticket_category_id = ticket_category_id ? String(ticket_category_id) : null;
    }
    if (admin_role_name !== undefined) serverPayload.admin_role_name = admin_role_name;
    if (transcript_channel_id !== undefined) {
      serverPayload.transcript_channel_id = transcript_channel_id ? String(transcript_channel_id) : null;
    }
