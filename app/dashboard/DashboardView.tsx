    ...
    async function loadGuilds() {
      try {
        ...
        setGuildId((currentGuildId) => {
          const currentExists =
            currentGuildId &&
            availableGuilds.some((guild) => guild.id === currentGuildId);

          return currentExists
            ? currentGuildId
            : availableGuilds[0]?.id ?? "";
        });
        ...
        if (!cancelled) {
          ...
          setGuildError(
            error instanceof Error
              ? error.message
              : "Não foi possível carregar os servidores do Discord.",
          );
        }
      } finally {
    ...
    switch (activeTab) {
      case "Overview":
        return (
          <OverviewTab
            data={data}
            onModerateSuggestion={handleModerateSuggestion}
          />
        );
      case "Inbox":
        return (
      ...
      case "Categories":
        return (
          <CategoriesTab
            data={data}
            onEdit={() => setActiveTab("Ticket Panels")}
          />
        );
      case "Staff":
      ...
      case "Settings":
        return (
          <SettingsTab
            data={data}
            onOpenPanelSettings={() => setActiveTab("Ticket Panels")}
          />
        );
      default:
        return null;
      ...
      <main className="min-w-0 flex-1 overflow-x-hidden">
        <div className="mx-auto max-w-[1600px] p-5 lg:p-8">
          <DashboardHeader
            title={activeTab}
            guildId={guildId}
            refreshing={loading || loadingGuilds}
            onRefresh={refresh}
          />
          ...
          {currentGuild && (
            <div className="sr-only">{currentGuild.name}</div>
          )}
