...
interface DashboardViewProps { user: { id: string; name: string; image: string | null }; initialData: DashboardMetrics; }
interface DashboardGuild { id: string; name: string; icon: string | null; owner?: boolean; }
interface Toast { id: string; type: "success" | "error"; message: string; }

const VALID_TABS = ["Overview", "Inbox", "Servers", "Ticket Panels", "Support Platform", "Categories", "Staff", "Transcripts", "Analytics", "Billing", "Settings"] as const;
...
export default function DashboardView({ user, initialData }: DashboardViewProps) {
  ...
  useEffect(() => {
    ...
    if (tab && (VALID_TABS as readonly string[]).includes(tab)) setActiveTab(tab);
  }, []);
  ...
  useEffect(() => {
    ...
    async function loadGuilds() {
      try {
        ...
        if (!cancelled) { setGuilds([]); setGuildError(error instanceof Error ? error.message : "Não foi possível carregar os servidores do Discord."); }
      } finally {
  ...
  async function handleModerateSuggestion(messageId: string, status: "Approved" | "Rejected") {
    try {
      const response = await fetch("/api/suggestions", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message_id: messageId, status, guild_id: guildId }) });
  ...
  function renderTab() {
    switch (activeTab) {
      ...
      case "Support Platform": return <PlatformTab guildId={guildId} />;
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 selection:bg-emerald-300/20">
      ...
      <div className="relative flex min-h-screen">
        ...
        <main className="min-w-0 flex-1 overflow-x-hidden"><div className="mx-auto max-w-[1700px] px-3 pb-24 pt-16 sm:px-5 sm:pt-20 lg:px-8 lg:pb-8 lg:pt-6"><DashboardHeader title={activeTab} guildId={guildId} refreshing={loading || loadingGuilds} onRefresh={refresh} />{visibleError && <div className="mb-5 rounded-2xl border border-red-400/15 bg-red-400/[0.04] px-4 py-3 text-sm text-red-300"><p className="font-medium">Não foi possível carregar os dados.</p><p className="mt-1 text-xs text-red-300/70">{visibleError}</p></div>}{guildId && <div className="mt-5 sm:mt-7"><AnimatePresence mode="wait"><motion.div key={`${guildId}-${activeTab}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }}>{renderTab()}</motion.div></AnimatePresence></div>}</div></main>
      </div>
    </div>
  );
}
