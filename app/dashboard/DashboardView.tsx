...
export default function DashboardView({ user, initialData }: DashboardViewProps) {
  ...
  const { data, loading, refresh } = useDashboard({ guildId, initialData });
  ...
  useEffect(() => {
    ...
    async function loadGuilds() {
      try {
        ...
        const response = await fetch("/api/dashboard/servers", { method: "GET", cache: "no-store" });
        if (!response.ok) throw new Error(`Failed to load guilds: ${response.status}`);
  ...
  return (
    <div className="flex min-h-screen bg-black text-zinc-100">
      ...
      <main id="dashboard-main" tabIndex={-1} className="min-w-0 flex-1 overflow-x-hidden focus:outline-none">
