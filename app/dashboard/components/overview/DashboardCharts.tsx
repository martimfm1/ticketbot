...
export function DashboardCharts({ data }: DashboardChartsProps) {
  ...
  const resolutionRate = totalTickets > 0 ? Math.round((data.tickets.closed / totalTickets) * 100) : 0;
  const approvalRate = totalSuggestions > 0 ? Math.round((data.suggestions.approved / totalSuggestions) * 100) : 0;
    ...
    <div className="grid gap-4 xl:grid-cols-[1.5fr_1fr_1fr]">
            ...
            return (
              <div key={point.key} className="group flex h-full flex-1 flex-col justify-end gap-2">
                <div className="relative flex flex-1 items-end">
                  <div className="w-full rounded-t-md bg-zinc-700 transition-all duration-300 group-hover:bg-zinc-500" style={{ height: `${height}%` }} title={`${point.value} ticket${point.value === 1 ? "" : "s"}`} />
                </div>
      ...
      <ChartCard title="Suggestions" description="Moderation status at a glance.">
        <div className="space-y-4 pt-5">
          <StackedBar segments={[{ label: "Approved", value: approvedPct }, { label: "Pending", value: pendingPct }, { label: "Rejected", value: rejectedPct }]} />
          ...
          <p className="text-[11px] text-zinc-600">Approval rate: <span className="text-zinc-400">{approvalRate}%</span></p>
        </div>
      </ChartCard>
    </div>
  );
  ...
  return (
    <div className="relative size-24 shrink-0">
      <svg viewBox="0 0 80 80" className="size-full -rotate-90">
        ...
        <circle cx="40" cy="40" r={radius} fill="none" stroke="currentColor" strokeWidth="7" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={offset} className="text-zinc-300 transition-all duration-500" />
      </svg>
      <div className="absolute inset-0 flex items-center justify-center"><span className="text-lg font-semibold text-zinc-100">{percentage}%</span></div>
    </div>
  );
}
...
function LegendRow({ label, value, percentage }: { label: string; value: number; percentage: number }) {
  return (
    <div>
      <div className="flex items-center justify-between gap-3 text-xs"><span className="text-zinc-400">{label}</span><span className="font-mono text-zinc-500">{value} · {Math.round(percentage)}%</span></div>
      <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-zinc-800"><div className="h-full rounded-full bg-zinc-500" style={{ width: `${Math.min(percentage, 100)}%` }} /></div>
    </div>
  );
}
...
function StackedBar({ segments }: { segments: Array<{ label: string; value: number }> }) {
  return (
    <div className="flex h-3 overflow-hidden rounded-full bg-zinc-800">
      {segments.map((segment) => <div key={segment.label} className="h-full bg-zinc-500 first:bg-zinc-300 last:bg-zinc-700" style={{ width: `${Math.max(segment.value, 0)}%` }} title={`${segment.label}: ${Math.round(segment.value)}%`} />)}
    </div>
  );
}

function MiniStat({ label, value }: { label: string; value: number }) {
  return <div className="rounded-lg border border-zinc-800/70 bg-zinc-950/40 p-2.5"><p className="text-[10px] text-zinc-600">{label}</p><p className="mt-1 text-base font-semibold text-zinc-200">{value}</p></div>;
}
