...
import { BarChart3, Bot, CreditCard, FileText, Inbox, LayoutDashboard, LogOut, Menu, Server, Settings, Ticket, Users, X, Sparkles } from "lucide-react";
...
export const dashboardNavigation = [
  { group: "Workspace", items: [
    ...
    { name: "Support Platform", icon: Bot },
  ] },
...
export interface DashboardGuild { id: string; name: string; icon: string | null; owner?: boolean; }
...
export function DashboardSidebar({ activeTab, guildId, guilds, user, onNavigate, onGuildChange }: DashboardSidebarProps) {
  ...
  useEffect(() => { if (!mobileOpen) return; const onKeyDown = (event: KeyboardEvent) => event.key === "Escape" && setMobileOpen(false); window.addEventListener("keydown", onKeyDown); return () => window.removeEventListener("keydown", onKeyDown); }, [mobileOpen]);

  const guildSelector = (compact = false) => <Select value={guildId} onValueChange={(value) => value && onGuildChange(value)} disabled={!guilds.length}>
    <SelectTrigger className={compact ? "h-11 min-w-0 flex-1 border-white/8 bg-white/[0.03] px-3 shadow-none focus:ring-0" : "h-12 w-full border-white/8 bg-white/[0.03] px-3 shadow-none hover:bg-white/[0.05] focus:ring-0"}>
      <SelectValue placeholder="Selecionar servidor">{currentGuild ? <div className="flex min-w-0 items-center gap-2.5"><div className="flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-zinc-800 text-xs font-semibold text-zinc-300">{currentGuild.icon ? <Image src={currentGuild.icon} alt="" width={32} height={32} className="size-full object-cover" /> : currentGuild.name.charAt(0).toUpperCase()}</div><div className="min-w-0 text-left"><p className="truncate text-xs font-medium text-zinc-200">{currentGuild.name}</p><p className="truncate font-mono text-[9px] text-zinc-600">{currentGuild.id}</p></div></div> : null}</SelectValue>
    </SelectTrigger>
    <SelectContent align="start" className="w-[min(320px,calc(100vw-2rem))] border-white/10 bg-zinc-950">{guilds.map((guild) => <SelectItem key={guild.id} value={guild.id} className="cursor-pointer py-3 focus:bg-white/[0.06] focus:text-zinc-100"><div className="flex items-center gap-2.5"><div className="flex size-8 items-center justify-center overflow-hidden rounded-lg bg-zinc-800 text-xs font-semibold text-zinc-300">{guild.icon ? <Image src={guild.icon} alt="" width={32} height={32} className="size-full object-cover" /> : guild.name.charAt(0).toUpperCase()}</div><span className="max-w-[220px] truncate text-xs">{guild.name}</span></div></SelectItem>)}</SelectContent>
  </Select>;
  ...
  return <>
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/6 bg-[#070707]/92 px-2 pb-[env(safe-area-inset-bottom)] pt-2 backdrop-blur-xl lg:hidden"><div className="mx-auto grid max-w-xl grid-cols-5 gap-1">{mobileItems.map((item) => { const Icon = item.icon; const active = activeTab === item.name; return <button key={item.name} type="button" onClick={() => onNavigate(item.name)} className={["flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl text-[10px] font-medium", active ? "bg-white/[0.06] text-white" : "text-zinc-500"].join(" ")}><Icon className="size-4" /><span>{item.name === "Ticket Panels" ? "Panels" : item.name === "Support Platform" ? "Platform" : item.name}</span></button>; })}</div></div>
