    ...
    <aside className="flex w-64 flex-col border-r border-zinc-800/80">
      ...
      <div className="border-b border-zinc-800/80 p-4">
        ...
        >
              ...
              {currentGuild && (
                <div className="flex min-w-0 items-center gap-2.5">
                  {currentGuild.icon ? (
                    ...
                    <div className="flex size-7 shrink-0 items-center justify-center rounded-md bg-zinc-800 text-xs font-semibold text-zinc-300">
          ...
          <SelectContent
            align="start"
            className="w-[230px] border-zinc-800 bg-zinc-950"
          >
            {guilds.length === 0 ? (
              <div className="px-3 py-6 text-center">
                <Server className="mx-auto mb-2 size-5 text-zinc-600" />
              ...
              guilds.map((guild) => (
                <SelectItem
                  key={guild.id}
                  value={guild.id}
                  className="cursor-pointer py-2.5 focus:bg-zinc-900 focus:text-zinc-100"
                >
                  <div className="flex min-w-0 items-center gap-2.5">
                    {guild.icon ? (
                      <Image
          ...
          {dashboardNavigation.map((item) => {
            ...
            return (
              <button
                ...
                className={[
                  "flex w-full cursor-pointer items-center gap-3 rounded-lg border px-3 py-2.5 text-xs font-medium transition-colors",
                  active
        ...
        <div className="flex items-center gap-3">
          {user.image ? (
            <Image
              ...
              width={32}
              height={32}
              className="size-8 shrink-0 rounded-full border border-zinc-800 object-cover"
            />
          ) : (
            <div className="flex size-8 shrink-0 items-center justify-center rounded-full border border-zinc-800 bg-zinc-900 text-xs font-medium text-zinc-300">
          ...
          <button
            ...
            className="cursor-pointer rounded-lg p-2 text-zinc-500 transition-colors hover:bg-zinc-900 hover:text-zinc-200"
            title="Logout"
            aria-label="Logout"
          >
