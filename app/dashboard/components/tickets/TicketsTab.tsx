...
import { CheckCircle2, CircleDot, Clock3, RotateCcw, UserRound, X } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-zinc-800/80 bg-zinc-900/40">
        <div className="flex flex-wrap items-center gap-2 border-b border-zinc-800/70 p-4">
          <div className="flex gap-2">
            {[
              ...
              <button
                ...
                className={`rounded-lg border px-3 py-2 text-xs ${statusFilter === value ? "border-zinc-700 bg-zinc-800 text-white" : "border-zinc-800 text-zinc-500 hover:text-zinc-200"}`}
              >
                {label}
              </button>
            ))}
          ...
          <select
            ...
            className="ml-auto rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2 text-xs text-zinc-300 outline-none"
          >
        ...
        <div className="grid min-h-[560px] lg:grid-cols-[minmax(0,420px)_1fr]">
          <div className="border-b border-zinc-800/70 lg:border-b-0 lg:border-r">
              ...
              {visibleTickets.length === 0 ? (
                ...
                visibleTickets.map((ticket) => (
                  <button
                    ...
                    className={`w-full border-b border-zinc-800/60 px-4 py-4 text-left transition-colors hover:bg-zinc-900/80 ${selected?.channelId === ticket.channelId ? "bg-zinc-900" : ""}`}
                  >
            ...
            {!selected ? (
              ...
              <div className="flex h-full min-h-[560px] flex-col">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-800/70 p-4">
                  <div>
                    <h2 className="text-sm font-semibold text-zinc-100">{selected.subject || "No subject"}</h2>
                    <p className="mt-1 font-mono text-[10px] text-zinc-600">Channel {selected.channelId} · User {selected.userId}</p>
                  </div>
                  <button type="button" onClick={() => setSelected(null)} className="rounded-lg p-2 text-zinc-600 hover:bg-zinc-900 hover:text-zinc-200" aria-label="Close details">
                    <X className="size-4" />
                  </button>
                </div>

                <div className="flex flex-wrap gap-2 border-b border-zinc-800/70 p-4">
                  <button disabled={Boolean(busyAction)} onClick={() => action("claim")} className="rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-2 text-xs text-zinc-300 hover:text-white disabled:opacity-50">Claim</button>
                  <button disabled={Boolean(busyAction)} onClick={() => action("unclaim")} className="rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-2 text-xs text-zinc-300 hover:text-white disabled:opacity-50">Unclaim</button>
                  <button disabled={Boolean(busyAction)} onClick={() => action("close")} className="rounded-lg border border-red-500/20 bg-red-500/10 px-3 py-2 text-xs text-red-400 hover:text-red-300 disabled:opacity-50">Close</button>
                  <button disabled={Boolean(busyAction)} onClick={() => action("reopen")} className="rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-400 hover:text-emerald-300 disabled:opacity-50">Reopen</button>
                  <button disabled={Boolean(busyAction)} onClick={() => action("priority", { priority: "urgent" })} className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-400 hover:text-amber-300 disabled:opacity-50">Urgent</button>
                  <button disabled={Boolean(busyAction)} onClick={() => action("priority", { priority: "normal" })} className="rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-2 text-xs text-zinc-300 hover:text-white disabled:opacity-50">Normal</button>
                  {selected.claimedBy && <span className="ml-auto inline-flex items-center gap-1 text-[10px] text-zinc-600"><CheckCircle2 className="size-3" />Assigned {selected.claimedBy}</span>}
                </div>
                ...
                <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
                  {loadingMessages ? (
                    ...
                    <p className="text-xs text-zinc-600">No persisted messages yet.</p>
                  ) : (
                    messages.map((message) => (
                      <div key={message.messageId} className="rounded-xl border border-zinc-800/70 bg-zinc-950/70 p-3">
                        <div className="flex items-center justify-between gap-3">
                          <p className="text-xs font-medium text-zinc-300">{message.authorName || message.authorId}</p>
                          <p className="text-[10px] text-zinc-600">{new Date(message.createdAt).toLocaleString()}</p>
                        </div>
