...
import { ArrowLeft, CheckCircle2, CircleDot, Clock3, UserRound, X } from "lucide-react";
...
export function TicketsTab({ data, guildId, onRefresh, onToast }: TicketsTabProps) {
  ...
  async function action(actionName: string, body: Record<string, unknown> = {}) {
    ...
    try {
      ...
      await onRefresh();
      onToast(result?.staleReference ? "Ticket atualizado; a referência do canal foi renovada." : "Ticket atualizado.");
    } catch (error) {
        ...
        <div className="flex flex-col gap-3 border-b border-zinc-800/70 p-3 sm:flex-row sm:flex-wrap sm:items-center sm:p-4">
          <div className="-mx-1 flex gap-1 overflow-x-auto px-1 pb-1 scrollbar-none sm:gap-2 sm:pb-0">
            {["all", "open", "pending", "closed"].map((value) => (
              <button key={value} type="button" onClick={() => setStatusFilter(value)} className={`min-h-10 shrink-0 rounded-lg border px-3 py-2 text-xs ${statusFilter === value ? "border-zinc-700 bg-zinc-800 text-white" : "border-zinc-800 text-zinc-500 hover:text-zinc-200"}`}>
                {statusLabel(value)}
              </button>
            ))}
          </div>
          <select value={priorityFilter} onChange={(event) => setPriorityFilter(event.target.value)} className="min-h-10 w-full rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2 text-xs text-zinc-300 outline-none sm:ml-auto sm:w-auto" aria-label="Filtrar por prioridade">
            <option value="all">All priorities</option>
            {priorityOrder.map((priority) => <option key={priority} value={priority}>{statusLabel(priority)}</option>)}
          </select>
        </div>
            ...
            <div className="max-h-[min(65vh,720px)] overflow-y-auto lg:max-h-[560px]">
              {visibleTickets.length === 0 ? (
                ...
                visibleTickets.map((ticket) => (
                  <button key={`${ticket.guildId}-${ticket.channelId}`} type="button" onClick={() => setSelected(ticket)} className={`min-h-[88px] w-full border-b border-zinc-800/60 px-3 py-4 text-left transition-colors active:bg-zinc-900 sm:px-4 ${selected?.channelId === ticket.channelId ? "bg-zinc-900" : "hover:bg-zinc-900/80"}`}>
          ...
          <div className={`${selected ? "block" : "hidden lg:block"} min-w-0`}>
            {!selected ? (
              <div className="flex min-h-[520px] items-center justify-center p-8 text-center">
                <div><UserRound className="mx-auto size-8 text-zinc-700" /><p className="mt-3 text-sm text-zinc-500">Select a ticket to view the conversation.</p></div>
              </div>
            ) : (
              <div className="flex min-h-[520px] flex-col lg:min-h-[560px]">
                <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-zinc-800/70 bg-zinc-950/95 p-3 backdrop-blur-xl sm:p-4">
                  <div className="flex min-w-0 items-center gap-2.5">
                    <button type="button" onClick={() => setSelected(null)} className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-zinc-800 bg-zinc-900 text-zinc-400 lg:hidden" aria-label="Voltar à lista de tickets"><ArrowLeft className="size-4" /></button>
                    <div className="min-w-0"><h2 className="truncate text-sm font-semibold text-zinc-100">{selected.subject || "No subject"}</h2><p className="mt-1 truncate font-mono text-[10px] text-zinc-600">#{selected.channelId} · {selected.userId}</p></div>
                  </div>
                  <button type="button" onClick={() => setSelected(null)} className="hidden size-10 shrink-0 items-center justify-center rounded-xl text-zinc-600 hover:bg-zinc-900 hover:text-zinc-200 lg:flex" aria-label="Close details"><X className="size-4" /></button>
                </div>
                ...
                <div className="min-h-[380px] flex-1 space-y-3 overflow-y-auto p-3 sm:p-4">
                  {loadingMessages ? <div className="animate-pulse space-y-3"><div className="h-20 rounded-xl bg-zinc-900/70" /><div className="ml-8 h-16 rounded-xl bg-zinc-900/70" /><div className="h-24 rounded-xl bg-zinc-900/70" /></div> : messages.length === 0 ? <div className="rounded-xl border border-dashed border-zinc-800 p-6 text-center text-xs text-zinc-600">No persisted messages yet.</div> : messages.map((message) => (
                    <div key={message.messageId} className="rounded-xl border border-zinc-800/70 bg-zinc-950/70 p-3.5 sm:p-3"><div className="flex items-start justify-between gap-3"><p className="min-w-0 truncate text-xs font-medium text-zinc-300">{message.authorName || message.authorId}</p><p className="shrink-0 text-[10px] text-zinc-600">{new Date(message.createdAt).toLocaleString()}</p></div><p className="mt-2 whitespace-pre-wrap break-words text-xs leading-5 text-zinc-400">{message.content || "[attachment]"}</p>{message.attachmentCount > 0 && <p className="mt-2 text-[10px] text-zinc-600">{message.attachmentCount} attachment(s)</p>}</div>
                  ))}
                </div>
              </div>
            )}
